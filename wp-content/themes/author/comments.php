 ...
 *
 */
_deprecated_file( sprintf( __( 'Theme without %1$s', 'okay' ), basename(__FILE__) ), '3.0', null, sprintf( __('Please include a %1$s template in your theme.', 'okay'), basename(__FILE__) ) );
...
<div id="comments">
	...
	<h3 id="comments-title">
		<?php printf( _n( __('One Response to %2$s', 'okay'), __('%1$s Responses to %2$s', 'okay'), get_comments_number() ), number_format_i18n( get_comments_number() ), '&#8220;' . get_the_title() . '&#8221;' ); ?>
	</h3>
...
<div id="respond">
	<h3><?php comment_form_title( __('Leave a Reply <span class="entypo">/</span>', 'okay'), __('Leave a Reply to %s', 'okay') ); ?></h3>
	...
	<p><?php printf(__('You must be <a href="%s">logged in</a> to post a comment.', 'okay'), wp_login_url( get_permalink() )); ?></p>
	<?php else : ?>
	...
	<p><small class="allowed"><strong>XHTML:</strong> <?php _e('You can use these tags:','okay'); ?><?php printf(__(' %s', 'okay'), allowed_tags()); ?></small></p>
	
	<p style="margin-bottom:0px;"><input name="submit" type="submit" class="respond-submit" id="submit" tabindex="5" value="<?php _e('Submit Comment'); ?>" />
