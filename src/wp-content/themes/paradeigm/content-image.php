...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?> data-postid="<?php the_ID(); ?>">
	...
	if ( !is_single() ) {
		printf( '<div class="post-format-icon"><a href="%1$s" title="%2$s"><i class="fa %3$s"></i></a></div><!-- .post-format-icon -->',
			...
			is_sticky() ? 'fa-thumb-tack' : 'fa-picture-o'
		);
	}
	?>
	...
	if ( '' != get_the_post_thumbnail() && !is_single() ) {
		printf( '<figure><a class="featured-image" href="%1$s" rel="bookmark"><div class="featured-info"><div class="info-wrapper">%2$s</div></div>%3$s</a></figure>',
			...
			__( 'View Image', 'hyphatheme' ),
			get_the_post_thumbnail( get_the_ID(), 'block-thumb' )
		);
	} else {
		if ( hypha_get_first_image() && !is_single() ) {
			printf( '<figure><a class="featured-image" href="%1$s" rel="bookmark"><div class="featured-info"><div class="info-wrapper">%2$s</div></div><img src="%1$s" alt="%2$s"></a></figure>',
				hypha_get_first_image(),
				__( 'View Image', 'hyphatheme' )
			);
		}
	}
	...
	<div class="entry-container">
		<?php if ( is_single() ) { ?>
			...
			<div class="entry-content">
				<?php
				printf( '<figure>%1$s</figure>', get_the_post_thumbnail( get_the_ID(), 'post-image' ) );
				the_content();
				wp_link_pages( array(
