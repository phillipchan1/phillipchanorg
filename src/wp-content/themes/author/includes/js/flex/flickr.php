...
function getFlickrPhotos($id, $limit=9) {
    ...
    foreach ($photos['photos']['photo'] as $photo) {
        $return.='<li><a class="lightbox" rel="flickr" href="' . $f->buildPhotoURL($photo, 'medium') . '" title="' . $photo['title'] . '"><img src="' . $f->buildPhotoURL($photo, 'medium') . '" alt="' . $photo['title'] . '" title="' . $photo['title'] . '" /></a></li>';
    }
    echo $return.='</ul>';
